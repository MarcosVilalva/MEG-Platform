...
export type RegistrationResult = AuthSession | { status: 'PENDING_APPROVAL'; administratorEmail: string };
...
export function readSession(): AuthSession | null {
  try { const value = localStorage.getItem(SESSION_KEY); return value ? JSON.parse(value) as AuthSession : null; } catch { return null; }
}
export function saveSession(session: AuthSession) { localStorage.setItem(SESSION_KEY, JSON.stringify(session)); }
...
export async function logout(session: AuthSession) {
  try { await request<void>('/auth/logout', { method: 'POST', body: JSON.stringify({ refreshToken: session.refreshToken }) }); }
...
export async function updateUserAccess(session: AuthSession, userId: string, changes: Partial<Pick<AuthUser, 'role' | 'approvalStatus' | 'isActive'>>) {
  return request<{ user: AuthUser }>(`/auth/users/${userId}/access`, {
    method: 'PATCH', headers: { Authorization: `Bearer ${session.accessToken}` }, body: JSON.stringify(changes)
  });
}