...
assert.doesNotMatch(nativeUpdate, /window\.setTimeout\(installAutomatically, 0\)/);
assert.match(nativeUpdate, />Atualizar agora<\/button>/);
assert.match(nativeUpdate, /Processando atualização…/);
assert.match(nativeUpdate, /toggleAttribute\('aria-busy', running\)/);
...
assert.equal(nativeUpdate.includes('appUpdateLater'), false, 'somente o fluxo canônico deve controlar a atualização Android');
