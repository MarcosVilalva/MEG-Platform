...
assert.match(server, /getNormalizationRuntimeStatus/);
assert.match(server, /15 \* 60 \* 1000/);
assert.match(server, /Periodic normalized financial source audit/);
assert.match(normalization, /fallbackCount/);
assert.match(normalization, /lastFallbackAt/);
...
assert.doesNotMatch(guard, /#closeDialogBtn/);
assert.doesNotMatch(guard, /#cancelDialogBtn/);
...
assert.doesNotMatch(legacyApp, /transactionMutationRunning \|\| window\.MEG_CLOUD_MUTATION_GUARD/);
