...
type DeliverySummary = {
  ...
  watchdog?: { lastCheckAt?: string | null; failedLast24Hours?: number; processing?: number; status?: 'ok' | 'attention' };
};
    ...
    <div className="px-settings-v15-layout">
      ...
      <div className="px-settings-workspace">
        ...
        {section === 'notifications' ? <>
          ...
          {data.user.role === 'ADMIN' ? <section className="px-settings-grid"><article className="px-card px-settings-card"><div className="px-settings-card-head"><div><span className="px-kicker">Entrega</span><h2>Últimas 24 horas</h2></div></div><dl><div><dt>Enviadas</dt><dd>{deliverySummary?.sentLast24Hours ?? '—'}</dd></div><div><dt>Falhas</dt><dd>{deliverySummary?.failedLast24Hours ?? '—'}</dd></div><div><dt>Último sucesso</dt><dd>{deliverySummary?.lastSuccessAt ? new Date(deliverySummary.lastSuccessAt).toLocaleString('pt-BR') : 'Não informado'}</dd></div><div><dt>Última falha</dt><dd>{deliverySummary?.lastFailureAt ? new Date(deliverySummary.lastFailureAt).toLocaleString('pt-BR') : 'Nenhuma registrada'}</dd></div><div><dt>Watchdog</dt><dd>{deliverySummary?.watchdog?.status === 'attention' ? 'Requer atenção' : deliverySummary?.watchdog?.lastCheckAt ? 'Ativo' : 'Aguardando primeiro ciclo'}</dd></div><div><dt>Último ciclo protegido</dt><dd>{deliverySummary?.watchdog?.lastCheckAt ? new Date(deliverySummary.watchdog.lastCheckAt).toLocaleString('pt-BR') : 'Ainda não executada'}</dd></div></dl></article><article className="px-card px-settings-card"><div className="px-settings-card-head"><div><span className="px-kicker">Agenda</span><h2>Horários atuais</h2><p>Os horários abaixo vêm da configuração ativa do servidor.</p></div></div><div className="px-settings-status-list"><span>Alertas · {notificationStatus?.automation?.schedule || 'Não informado'}</span><span>Alexa · {notificationStatus?.alexa?.schedule || 'Não informado'}</span></div><p className="px-settings-note">A próxima etapa desta tela será permitir editar essas agendas e o período silencioso sem expor credenciais.</p></article></section> : null}
        </> : null}
