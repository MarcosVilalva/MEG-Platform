...
export function PhoenixPayables({ data, onMonthChange, onEditEvent }: { data: PhoenixReadModel; onMonthChange?: (month: string) => void; onEditEvent?: (eventId: string) => void }) {
  ...
  async function confirmSettlement() {
    ...
    try {
      ...
      setWriteState({
        ...
        message: 'Não foi possível concluir a baixa. O MEG não marcou o lote como concluído; você pode tentar novamente com segurança.'
      });
    }
  }
  ...
  return <section className="px-screen px-pending-cockpit" data-pending-layout="canonical-v3" onWheel={routeWheelToPendingList}>
    ...
    <nav className="px-pending-month-nav" aria-label="Período das pendências">
      <button type="button" className="px-pending-month-step" aria-label="Mês anterior" onClick={() => selectPendingMonth(shiftPendingMonth(selectedMonth, -1))}>‹</button>
