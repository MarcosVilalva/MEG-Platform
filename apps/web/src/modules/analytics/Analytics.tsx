...
export function Analytics() {
  ...
  return <section id="analytics" className="page analytics-page" aria-busy={loading}>
    <header className="page-head page-header analytics-hero compact"><div><span>Análises</span><h1>Tendência e comparação histórica</h1><p>O filtro analítico não pode esconder um déficit existente no mês corrente. A Home permanece responsável pelas decisões imediatas.</p></div>{periodMode === 'month' && <label className="catalog-role">Período<input type="month" value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)} /></label>}</header>
