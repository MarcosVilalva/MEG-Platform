...
export function Dashboard({ onNewTransaction: _onNewTransaction }: { onNewTransaction: () => void }) {
  ...
  return <section className="meg-screen dashboard-screen" aria-busy={loading}>
    ...
    <section className="balance-card"><div><span>SALDO MONETÁRIO REALIZADO</span><strong>{money.format(summary?.realizedResult || 0)}</strong><p>Receita disponível menos despesas monetárias efetivamente pagas.</p></div><dl><div><dt>Saldo anterior</dt><dd>{money.format(summary?.availableBalance || 0)}</dd></div><div><dt>Receitas do mês</dt><dd>{money.format(summary?.income || 0)}</dd></div><div><dt>Receita disponível</dt><dd>{money.format((summary?.availableBalance || 0) + (summary?.income || 0))}</dd></div></dl></section>
