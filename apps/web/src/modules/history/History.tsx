...
export function History() {
  ...
  return <section className="meg-screen history-screen" aria-busy={loading}>
    <header className="screen-heading"><div><span>HISTÓRICO</span><h1>Atividade dos lançamentos</h1><p>Consulta separada do formulário, do mais novo para o mais antigo.</p></div></header>
    {error && <div className="notice danger">{error}</div>}
    <section className="history-summary"><article><span>ATIVIDADES NO PERÍODO</span><strong>{items.length}</strong></article><article><span>USUÁRIO ATIVO</span><strong>{userName}</strong></article><article><span>AGUARDANDO SINCRONIZAÇÃO</span><strong className="positive">0</strong></article></section>
