...
export function PersistentTransactions() {
  ...
  return <section id="movements" className="page meg-screen transactions-screen">
    ...
    <article className="card movement-card transaction-workspace">
      ...
      <div className="launch-toolbar transaction-filter-panel"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar descrição, grupo, conta ou pagamento"/><select value={filterType} onChange={(e) => setFilterType(e.target.value)}><option value="all">Todos os tipos</option><option value="income">Receitas</option><option value="expense">Despesas</option></select><select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}><option value="all">Todas as situações</option><option value="planned">Pendentes</option><option value="paid">Pagos/recebidos</option><option value="reconciled">Conciliados</option></select><select value={filterGroup} onChange={(e) => setFilterGroup(e.target.value)}><option value="all">Todos os grupos</option>{groups.map((x) => <option key={x}>{x}</option>)}</select><select value={filterAccount} onChange={(e) => setFilterAccount(e.target.value)}><option value="all">Todas as contas</option>{accountNames.map((x) => <option key={x}>{x}</option>)}</select><select value={order} onChange={(e) => setOrder(e.target.value as typeof order)}><option value="newest">Mais recentes</option><option value="oldest">Mais antigos</option><option value="highest">Maior valor</option></select></div>
