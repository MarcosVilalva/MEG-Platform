...
export function CreditCards() {
  ...
  const [detailTab, setDetailTab] = useState<'invoices' | 'purchases'>('invoices');
  ...
  return <section id="cards" className="page cards-page validated-cards">
    <header className="page-head page-header compact"><div><span>Cartões de crédito</span><h1>Faturas e compromissos</h1><p>Acompanhe cada cartão sem misturar fatura atual, compras após o fechamento e parcelas futuras.</p></div><div className="cards-head-actions"><label className="catalog-role">Mês<input type="month" value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)} /></label>{canWrite && <><button className="meg-icon-action" title="Gerenciar cartões" aria-label="Gerenciar cartões" onClick={() => setEditor('card')}>＋</button><button className="header-primary" onClick={() => setEditor('purchase')} disabled={!cards.length}>＋ Compra</button></>}</div></header>
    ...
    {loading && !cards.length ? <div className="meg-card empty-state">Carregando cartões da base...</div> : selectedCard ? <>
      ...
      <nav className="cards-view-tabs"><button className={detailTab === 'invoices' ? 'active' : ''} onClick={() => setDetailTab('invoices')}>Faturas e projeção</button><button className={detailTab === 'purchases' ? 'active' : ''} onClick={() => setDetailTab('purchases')}>Lançamentos do cartão</button></nav>
      {detailTab === 'invoices' ? <article className="meg-card card-forecast"><header><div><span>PROJEÇÃO</span><h3>Próximos seis meses</h3></div><strong>{brl.format(forecast.reduce((sum, item) => sum + item.amount, 0))}</strong></header><div className="invoice-summary"><div><span>Fatura selecionada</span><strong>{brl.format(selectedCard.statementAmount)}</strong></div><div><span>Compras em aberto</span><strong>{selectedCard.purchases.length}</strong></div><div><span>Vencimento</span><strong>Dia {selectedCard.dueDay}</strong></div></div><div className="forecast-bars">{forecast.map((item) => <div key={item.month}><strong>{brl.format(item.amount)}</strong><span><i style={{ height: `${Math.max(item.amount ? 8 : 2, item.amount / forecastMax * 100)}%` }} /></span><small>{shortMonth(item.month)}</small></div>)}</div></article> : <article className="meg-card card-purchase-ledger"><header><div><span>COMPRAS DO PERÍODO</span><h3>{selectedCard.name}</h3></div><strong>{selectedCard.purchases.length} compra(s)</strong></header><div className="card-purchases">{selectedCard.purchases.slice(0, 50).map((purchase) => <div key={purchase.id}><span><strong>{purchase.description}</strong><small>{purchase.installments}x · {new Date(purchase.purchaseDate).toLocaleDateString('pt-BR')}</small></span><strong>{brl.format(Number(purchase.totalAmount))}</strong>{canDelete && purchase.status !== 'legacy' && <button className="danger" onClick={() => void cardsClient.cancelPurchase(purchase.id).then(load)}>Cancelar</button>}</div>)}{!selectedCard.purchases.length && <div className="empty-state">Nenhuma compra neste período.</div>}</div></article>}
