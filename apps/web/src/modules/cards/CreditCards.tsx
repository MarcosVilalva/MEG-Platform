    ...
    {loading && !cards.length ? <div className="card empty-state">Carregando cartões da base...</div> : selectedCard ? <div className="cards-shell">
      ...
      <div className="cards-detail">
        <article className="card"><div className="card-hero-grid">
          <div className={`physical-card is-${selectedAsset ? 'generic' : 'generic'}`} style={{ background: `linear-gradient(135deg, ${selectedCard.color || '#756d62'}, #071727)` } as CSSProperties}><div className="card-art card-art-generic" style={{ display: 'block' }}><strong className="generic-card-brand">{selectedCard.issuer || 'MEG'}</strong><span className="generic-card-chip"/><span className="generic-card-label">{selectedCard.brand || 'CARTÃO CADASTRADO'}</span>{selectedAsset && <img className="card-brand-logo" src={`${import.meta.env.BASE_URL}assets/card-brands/${selectedAsset}.svg`} alt={selectedCard.brand || selectedAsset}/>}</div></div>
