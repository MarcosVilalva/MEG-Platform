...
export function CreditCards() {
  ...
  async function createCard(event: FormEvent) {
    ...
    const value = parseBRL(limit);
    if (!canWrite || !cardName.trim() || !Number.isFinite(value) || value <= 0) return;
    setBusy(true);
    try {
      await cardsClient.create({ name: cardName.trim(), issuer: issuer.trim() || undefined, creditLimit: value, closingDay: Number(closingDay), dueDay: Number(dueDay) });
      setCardName(''); setIssuer(''); setLimit(''); await load();
  ...
  async function createPurchase(event: FormEvent) {
    ...
    if (!canWrite || !cardId || !description.trim() || !Number.isFinite(value) || value <= 0) return;
    setBusy(true);
    try {
      await cardsClient.createPurchase({ cardId, categoryId: categoryId || undefined, description: description.trim(), totalAmount: value, purchaseDate, installments: Number(installments) });
      setDescription(''); setAmount(''); setInstallments('1'); await load();
