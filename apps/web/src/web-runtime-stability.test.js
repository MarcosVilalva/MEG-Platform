...
assert.match(comfortContrast, /\.sidebar-user-footer/);
assert.match(comfortContrast, /\.sidebar-version-status/);
assert.match(comfortContrast, /#transactionDialog/);
assert.match(comfortContrast, /\.ongoing-installment-panel/);
assert.match(comfortContrast, /\.meg-stable-filter-popover/);
assert.match(comfortContrast, /#androidBiometricSettingsBtn/);
assert.match(comfortContrast, /\.meg-kpi-card/);
assert.match(comfortContrast, /\.meg-chart-card/);
assert.match(comfortContrast, /\.meg-pivot-card/);
assert.match(comfortContrast, /\.meg-mobile-transaction-icon/);
assert.match(comfortContrast, /\.meg-mobile-status/);
assert.match(comfortContrast, /\.filter-clear-button/);
assert.match(comfortContrast, /\.user-status-pill/);
assert.match(comfortContrast, /\.primary-admin-badge/);
assert.match(comfortContrast, /\.license-status/);
assert.match(comfortContrast, /\.commercial-license-controls/);
assert.match(comfortContrast, /\.analytics-filter-copy/);
assert.match(comfortContrast, /\.analytics-equation > b/);
assert.match(comfortContrast, /\.executive-score/);
assert.match(comfortContrast, /\.meg-stable-filter-button/);
assert.match(comfortContrast, /\.workspace-commercial-summary/);
assert.match(comfortContrast, /\.user-avatar/);
assert.match(comfortContrast, /\.user-security-hero/);
assert.match(indexHtml, /class="[^"]*sidebar-user-footer[^"]*"/);
assert.match(indexHtml, /Versão instalada/);
assert.match(indexHtml, /class="sidebar-user-actions"/);
assert.match(legacyApp, /`APK v\$\{window\.MEG_INSTALLED_APP_VERSION\.versionName\}`/);
assert.doesNotMatch(legacyApp, /MEG v1\.3\.0/);
...
assert.doesNotMatch(biometricSettings, /background:\s*#eff8f5/);
