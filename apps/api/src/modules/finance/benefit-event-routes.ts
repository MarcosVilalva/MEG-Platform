...
import { z } from 'zod';
import { BenefitEventMutationError, createBenefitEventProtected } from './benefit-event-mutation';
...
function benefitError(reply: FastifyReply, error: unknown) {
  ...
  const status = ['OPERATION_ID_REUSED', 'INSUFFICIENT_BENEFIT_BALANCE'].includes(error.code) ? 409
