...
function benefitOpeningBalance(accounts: Array<{ type: string; openingBalance: unknown }>) {
  return round(accounts
    .filter((account) => normalizeText(account.type) === 'BENEFIT')
    .reduce((sum, account) => sum + Number(account.openingBalance || 0), 0));
}

function periodTotals(events: CoreEvent[]) {
  let income = 0;
  let expense = 0;
...
function buildSummary(
  month: string,
  monthEvents: CoreEvent[],
  historicalEvents: CoreEvent[],
  openingBalance: number,
  futureCandidates: FutureCandidate[],
) {
  ...
  const nextDue = futureCandidates.find((event) => isMonetaryEvent(event)
    && event.type !== 'income'
    && event.type !== 'redemption') || null;
  const topCategories = [...totals.categoryTotals.entries()]
  ...
  return {
    ...
    projectedResult: totals.projectedResult,
...
function buildCashflow(month: string, monthEvents: CoreEvent[], openingBalance: number) {
  const events = monthEvents.filter(isMonetaryEvent);
...
function buildAnalytics(month: string, currentSummary: ReturnType<typeof buildSummary>, currentEvents: CoreEvent[], allEvents: CoreEvent[]) {
  const [year, monthNumber] = month.split('-').map(Number);
  ...
  const previousEvents = allEvents.filter((event) => event.date >= previousStart && event.date < currentStart);
  const previousTotals = periodTotals(previousEvents);
  const events = currentEvents.filter(isMonetaryEvent);
  const trendEvents = allEvents.filter((event) => event.date >= trendStart && isMonetaryEvent(event));
  ...
  return {
    ...
    previous: { month: previousMonth, income: previousTotals.income, expense: previousTotals.expense, result: previousTotals.projectedResult },
    delta: {
      income: round(currentSummary.income - previousTotals.income),
      expense: round(currentSummary.expense - previousTotals.expense),
      result: round(currentSummary.projectedResult - previousTotals.projectedResult),
    },
    dailyAverageExpense: expenseDays.size ? round(currentSummary.expense / expenseDays.size) : 0,
...
function buildBenefitSummary(month: string, allEvents: CoreEvent[], monthEvents: CoreEvent[], openingBalance: number) {
...
type WorkspaceContext = Awaited<ReturnType<typeof resolveWorkspaceContext>>;

async function cardsReadOnly(context: WorkspaceContext, month: string) {
  ...
  return cards.map((card) => {
    ...
    const payableStatementAmount = entries.filter((entry) => entry.statementMonth === month && entry.status === 'open')
...
async function financialAuditReadOnly(context: WorkspaceContext) {
  const members = await prisma.workspaceMember.findMany({ where: { workspaceId: context.workspaceId }, select: { userId: true } });
  ...
  return {
    items: items.map((item) => ({
      ...
      action: item.action,
      ...parseAuditMetadata(item.metadata),
    })),
    total,
    page: 1,
    pageSize: 100,
...
async function loadCoreEvents(userId: string, end: Date): Promise<CoreEvent[]> {
...
async function loadFutureCandidates(userId: string): Promise<FutureCandidate[]> {
  return prisma.financialEvent.findMany({
    where: { userId, archivedAt: null, status: 'planned', date: { gte: new Date() } },
    orderBy: [{ date: 'asc' }, { createdAt: 'asc' }],
    take: 100,
    select: {
      id: true,
  ...
  }) as Promise<FutureCandidate[]>;
}

/**
 * Snapshot Phoenix otimizado e estritamente somente leitura.
 *
 * Diferente da implementação inicial, resumo, análises, fluxo e benefício são
 * derivados da mesma leitura histórica mínima. Isso evita recalcular o mesmo mês
 * várias vezes e elimina consultas repetidas de saldo anterior durante uma única
 * troca de período.
 */
export async function getPhoenixPreviewSnapshot(userId: string, month: string) {
  ...
  const monthEvents = allEvents.filter((event) => event.date >= start);
  const historicalEvents = allEvents.filter((event) => event.date < start);
  const openingBalance = monetaryOpeningBalance(accounts);
  const summaryBase = buildSummary(month, monthEvents, historicalEvents, openingBalance, futureCandidates);
  const benefit = buildBenefitSummary(month, allEvents, monthEvents, benefitOpeningBalance(accounts));
  const cashflow = buildCashflow(month, monthEvents, summaryBase.availableBalance);
  const analytics = buildAnalytics(month, summaryBase, monthEvents, allEvents);
  ...
  return {
    ...
    sourcePolicy: {
      mode: 'read-only',
      mutationFree: true,
      aggregation: 'single-core-event-read',
