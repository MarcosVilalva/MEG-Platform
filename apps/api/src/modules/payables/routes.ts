...
import { createPayablePaymentProtected, PayablePaymentMutationError } from './payment-mutation';
...
const operationIdSchema = z.string().trim().min(8).max(120).regex(/^[A-Za-z0-9._:-]+$/);
...
const paymentSchema = z.object({ amount: z.coerce.number().positive(), paidAt: z.string().min(10), interestAmount: z.coerce.number().min(0).default(0), fineAmount: z.coerce.number().min(0).default(0), accountId: z.string().optional().nullable(), paymentMethodId: z.string().optional().nullable(), notes: z.string().max(500).optional().nullable(), operationId: operationIdSchema.optional() });
...
export async function payableRoutes(app: FastifyInstance) {
  ...
  app.post('/:id/payments', { preHandler: app.authorize([...writeRoles]) }, async (request, reply) => {
    const parsed = paymentSchema.safeParse(request.body); if (!parsed.success) return validationError(reply, parsed.error.flatten());
    const { id } = request.params as { id: string };
    try {
      const result = await createPayablePaymentProtected(request.user.sub, { payableId: id, ...parsed.data });
      return reply.code(201).send(result);
    } catch (error) {
      if (!(error instanceof PayablePaymentMutationError)) throw error;
      if (error.code === 'PAYABLE_NOT_FOUND') return reply.code(404).send({ error: error.code });
      if (error.code === 'OPERATION_ID_REUSED') return reply.code(409).send({ error: error.code });
      if (['AMOUNT_EXCEEDS_OPEN_BALANCE', 'INVALID_ACCOUNT', 'INVALID_PAYMENT_METHOD'].includes(error.code)) {
