...
const paymentLabel = (item: LegacyTransaction) => String(item.paymentMethod || item.account || 'NÃO INFORMADO').toUpperCase();

function saoPauloDate(referenceDate: Date) {
  const parts = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Sao_Paulo', year: 'numeric', month: '2-digit', day: '2-digit' }).formatToParts(referenceDate);
  const part = (type: string) => parts.find((item) => item.type === type)?.value || '';
  const iso = `${part('year')}-${part('month')}-${part('day')}`;
  return { iso, date: new Date(`${iso}T12:00:00-03:00`) };
}

export function buildNotificationDigest(transactions: LegacyTransaction[], referenceDate = new Date()) {
  const local = saoPauloDate(referenceDate);
  const today = local.date;
  const limit = new Date(today);
  limit.setDate(limit.getDate() + 3);
  const pending = transactions.filter((item) => {
    if (String(item.type || '').toLowerCase() !== 'expense' || !item.date) return false;
  ...
  const overdue = pending.filter((item) => new Date(`${item.date}T12:00:00-03:00`) < today);
  const upcoming = pending.filter((item) => new Date(`${item.date}T12:00:00-03:00`) >= today);
  ...
  if (overdue.length) {
    ...
    overdue.forEach((item) => lines.push(`  ⚠️ *${String(item.description || 'Conta sem descrição').toUpperCase()}*`, `     📆 ${dateLabel(item.date!)} · 💵 ${money(Number(item.expenseAmount ?? item.amount ?? 0))} · 🏦 ${paymentLabel(item)}`));
  }
  if (upcoming.length) {
    lines.push('', '🟡 *PRÓXIMOS 3 DIAS — Já deixa no radar!* 👀');
    upcoming.forEach((item) => lines.push(`  ⏰ *${String(item.description || 'Conta sem descrição').toUpperCase()}*`, `     📆 ${dateLabel(item.date!)} · 💵 ${money(Number(item.expenseAmount ?? item.amount ?? 0))} · 🏦 ${paymentLabel(item)}`));
  }
  lines.push('', '━━━━━━━━━━━━━━━━━━━━', '💡 *Dica do dia:* Conta paga é conta que não te tira o sono! 😴✅', '', '🤖 *MEG Finanças — Seu copiloto financeiro*');
  return { overdue, upcoming, text: lines.join('\n'), totalCount: pending.length, totalAmount: total(pending) };
}
...
export async function notificationDigest(userId: string, referenceDate = new Date()) {
  ...
  return buildNotificationDigest(state?.transactions || [], referenceDate);
}

async function sendEmail(subject: string, text: string) {
  if (!config.resendApiKey) return { status: 'skipped', detail: 'RESEND_API_KEY não configurada' };
  const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${config.resendApiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: config.notificationEmailFrom, to: [config.adminEmail], subject, text }) });
...
async function sendWhatsApp(number: string, text: string) {
  ...
  const response = await fetch(`${config.evolutionApiUrl.replace(/\/$/, '')}/message/sendText/${encodeURIComponent(config.evolutionInstance)}`, { method: 'POST', headers: { apikey: config.evolutionApiKey, 'Content-Type': 'application/json' }, body: JSON.stringify({ number: number.replace(/\D/g, ''), text }) });
...
export function notificationIntegrationStatus() {
  return {
    ...
    automation: { configured: Boolean(config.notificationCronSecret), schedule: '08:00 America/Sao_Paulo' }
  };
}
...
export async function deliverNotifications(userId: string, force = false, recipientIds: string[] = []) {
  ...
  const recipients = await prisma.notificationRecipient.findMany({ where: { userId, isActive: true, ...(recipientIds.length ? { id: { in: recipientIds } } : {}) }, orderBy: { name: 'asc' } });
  const whatsappTargets = recipients.length ? recipients.map((recipient) => ({ id: recipient.id, name: recipient.name, phone: recipient.phone })) : config.whatsappRecipient ? [{ id: 'default', name: 'Número padrão', phone: config.whatsappRecipient }] : [];
  const channels = [
  ...
  for (const item of channels) {
    ...
    const existing = await prisma.notificationDelivery.findUnique({ where: { userId_channel_reference: { userId, channel: item.channel, reference: key } } });
    ...
    try {
      ...
      if (result.status === 'sent') await prisma.notificationDelivery.upsert({ where: { userId_channel_reference: { userId, channel: item.channel, reference: key } }, create: { userId, channel: item.channel, reference: key, status: result.status, detail: result.detail }, update: { status: result.status, detail: result.detail, deliveredAt: new Date() } });
